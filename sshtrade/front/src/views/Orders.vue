  ...
  <div>
    ...
    <div style="margin: 10px 0">
      <el-table :data="state.tableData" stripe border  @selection-change="handleSelectionChange">
        ...
        <el-table-column label="下单用户"><template #default="scope"><span v-if="scope.row.userId">{{ state.userOptions.find(v => v.id === scope.row.userId) ? state.userOptions.find(v => v.id === scope.row.userId).name : '' }}</span></template></el-table-column>
        <el-table-column prop="amount" label="总金额"></el-table-column>
        <el-table-column label="商品发布人"><template #default="scope"><span v-if="scope.row.bizUserId">{{ state.userOptions.find(v => v.id === scope.row.bizUserId) ? state.userOptions.find(v => v.id === scope.row.bizUserId).name : '' }}</span></template></el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" title="商品订单信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item label-width="100" prop="name" label="订单号">
          <el-input v-model="state.form.name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label-width="100" prop="content" label="订单明细">
        ...
        <el-form-item label-width="100" prop="stateRadio" label="订单状态">
          <el-radio-group v-model="state.form.stateRadio">
            <el-radio label="已下单">已下单</el-radio>
            <el-radio label="已发货">已发货</el-radio>
            <el-radio label="已收货">已收货</el-radio>
            <el-radio label="已取消">已取消</el-radio>
          </el-radio-group>        </el-form-item>
        <el-form-item label-width="100" prop="amount" label="总金额">
          <el-input v-model="state.form.amount" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label-width="100" prop="goodids" label="产品编号">
          <el-input v-model="state.form.goodids" autocomplete="off"></el-input>
        </el-form-item>
